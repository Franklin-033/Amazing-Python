pipeline {
  ...
  environment {
    ...
    DOCKER_IMAGE = 'amazing-python-app-2023mcs320002pr' 
}
        ...
        stage('Docker Run') {
            steps {
                sh '''
                    ...
                    docker run --name amazing-python-app-2023mcs320002pr -p 5000:5000 ${DOCKER_IMAGE}
                '''
            }
        }
    ...
    steps {
        withCredentials([usernamePassword(credentialsId: 'jenkins-doccker', usernameVariable: 'DOCKERHUB_USER', passwordVariable: 'DOCKERHUB_PASS')]) {
            sh '''
                ...
                docker tag amazing-python-app:latest $DOCKERHUB_USER/amazing-python-app2023mcs320002pr:latest
                docker push $DOCKERHUB_USER/amazing-python-app-2023mcs320002pr:latest
            '''
        }
    }
}
